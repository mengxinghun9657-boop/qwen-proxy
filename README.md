...
# 通过管道传入代码
cat buggy_code.py | ./venv/bin/python ask_qwen.py -s "你是代码审查专家，找出所有 bug"

# 指定更强模型
./venv/bin/python ask_qwen.py -m qwen3.6-max-preview "分析这段架构设计的优缺点"

# 多轮对话
./venv/bin/python ask_qwen.py -c debug-session "这个 NullPointerException 是什么原因？"
./venv/bin/python ask_qwen.py -c debug-session "你建议怎么修？"

# 流式输出
./venv/bin/python ask_qwen.py --stream "写一个快速排序的 Python 实现"
...
- **会话持久化**: 通过 `-c` 参数自动保存/恢复会话状态，文件存储在同目录下

*Design principles: single-file with minimal dependencies; pipe-friendly (stdin/stdout for Unix pipeline integration); session persistence via `-c` flag.*
